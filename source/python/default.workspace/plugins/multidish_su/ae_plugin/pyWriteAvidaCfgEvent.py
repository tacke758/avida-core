...
class pyWriteAvidaCfgEvent:
  ...
  def __init__(self, in_dict = None, session_mdl = None, workspace_dir = None, 
    ...
    print "pyWriteAvidaCfgEvent.__init__(): writing environment file to " + tmp_in_dir
    ...
    self.avida_cfg_file_name = self.writeAvidaCfgFile(workspace_dir, tmp_in_dir, \
                                              settings_dict)
    ...
    #   merit as value.
    self.avida_cfg = cGenesis()

    cConfig.Setup(self.avida_cfg)

    self.avida_cfg.Open(cString(self.avida_cfg_file_name))
