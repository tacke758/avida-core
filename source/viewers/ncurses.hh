...
protected:
  WINDOW * win_id;
public:
  cTextWindow(int y_size=0, int x_size=0, int y_start=0, int x_start=0);
  ~cTextWindow();
  ...
  inline void Redraw() { touchwin(win_id); wrefresh(win_id); }
  ...
  inline void Refresh() { wrefresh(win_id); }

  // These functions clear sections of the screen.
  inline void Clear() { wclear(win_id); }
