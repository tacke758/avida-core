...
void cZoomScreen::DrawStats()
{
  ...
  for (int cur_col = TASK_X + 2;
    ...
    for (int cur_row = TASK_Y + 1;
	 cur_row <= TASK_Y + 10 && task_num < cConfig::GetNumTasks();
	 cur_row++) {
  ...
  /*
  Print(TASK_Y + 1, TASK_X + 2, "Get.....:");
  Print(TASK_Y + 2, TASK_X + 2, "Put.....:");
  Print(TASK_Y + 3, TASK_X + 2, "GGP.....:");
  Print(TASK_Y + 4, TASK_X + 2, "Echo....:");
  Print(TASK_Y + 5, TASK_X + 2, "Nand....:");
  Print(TASK_Y + 6, TASK_X + 2, "Not.....:");
  Print(TASK_Y + 7, TASK_X + 2, "And.....:");
  Print(TASK_Y + 8, TASK_X + 2, "~XorY...:");
  Print(TASK_Y + 9, TASK_X + 2, "~XandY..:");
  Print(TASK_Y + 10, TASK_X + 2, "Or......:");

  Print(TASK_Y + 1, TASK_X + 16, "Nor.....:");
  Print(TASK_Y + 2, TASK_X + 16, "Xor.....:");
  Print(TASK_Y + 3, TASK_X + 16, "Equ.....:");
  */
}
...
void cZoomScreen::UpdateStats()
{
  ...
  // Flags...
  if (info.GetActiveCPU()->GetFlag(CPU_FLAG_MAL_ACTIVE))
    SetBoldColor(COLOR_CYAN);
  else SetColor(COLOR_CYAN);
  Print(CPU_FLAGS_Y + 1, CPU_FLAGS_X + 1, "Mem Allocated");

  if (info.GetActiveCPU()->GetFlag(CPU_FLAG_PARENT_TRUE))
    SetBoldColor(COLOR_CYAN);
  else SetColor(COLOR_CYAN);
  Print(CPU_FLAGS_Y + 2, CPU_FLAGS_X + 1, "Parent True");

  if (info.GetActiveCPU()->GetFlag(CPU_FLAG_INJECTED))
    SetBoldColor(COLOR_CYAN);
  else SetColor(COLOR_CYAN);
  Print(CPU_FLAGS_Y + 3, CPU_FLAGS_X + 1, "Injected");

  if (info.GetActiveCPU()->GetFlag(CPU_FLAG_PARASITE))
    SetBoldColor(COLOR_CYAN);
  else SetColor(COLOR_CYAN);
  Print(CPU_FLAGS_Y + 4, CPU_FLAGS_X + 1, "Parasite");

  if (info.GetActiveCPU()->GetFlag(CPU_FLAG_POINT_MUT))
    SetBoldColor(COLOR_CYAN);
  else SetColor(COLOR_CYAN);
  Print(CPU_FLAGS_Y + 5, CPU_FLAGS_X + 1, "Point Mutated");
      
  if (info.GetActiveCPU()->GetFlag(CPU_FLAG_MODIFIED))
    SetBoldColor(COLOR_CYAN);
  else SetColor(COLOR_CYAN);
  ...
  if (info.GetPauseLevel() && info.GetActiveCPU() &&
      info.GetActiveCPU()->GetFlag(CPU_FLAG_PARASITE)) {
...
void cZoomScreen::UpdateCPU()
{
  ...
  SetColor(COLOR_WHITE);
  if (info.GetActiveCPU()->GetActiveStack() == 0) {
    Print(STACK_Y + 1, STACK_X + 2, "Stack A");
  } else {
    Print(STACK_Y + 1, STACK_X + 2, "Stack B");
  }
  ...
  for (i = -MEMORY_PRE_SIZE; i <= MEMORY_POST_SIZE; i++) {
	...
	if (memory.GetFlag(adj_inst_ptr, INST_FLAG_EXECUTED)) {
	  Print(MEMORY_Y + MEMORY_PRE_SIZE + 3 + i, MEMORY_X + 25, "Exe");
	}
	if (memory.GetFlag(adj_inst_ptr, INST_FLAG_BREAKPOINT)) {
	  Print(MEMORY_Y + MEMORY_PRE_SIZE + 3 + i, MEMORY_X + 30, "Brk");
	}
      }
