...
void cTextWindow::Print(char * fmt, ...)
{
...
void cTextWindow::Print(int in_y, int in_x, char * fmt, ...)
{
...
static	cQueue *pcInputQueue;	// "global" input queue (local to this module)
...
void StartProg()
{
  ...
  pcInputQueue = new cQueue;	// initialize our input queue
					...
					case MOUSE_EVENT:
							...
							))
						{
							...
							else
							{
								...
								if (pcInputQueue->QueueAdd(pmiData) != 0)
									// It's in the queue
...
{
	return (MOUSE_INFO *)pcInputQueue->QueueGet();
}

void FlushMouseQueue(void)
{
	pcInputQueue->QueueFlush();
}
