  ...
  info( in_population )
{
  ...
  info.SetActiveCPU(& (in_population.GetCPU(0)));
}
...
void cView::Setup(const cString & in_name)
{
  ...
  bar_screen = new cBarScreen(3, 0, 0, 0, info, in_name);
...
{
  if (info.GetPauseLevel() == PAUSE_ADVANCE_STEP) return;
  NotifyUpdate();
}
...
void cView::NotifyUpdate()
{
  ...
  if (info.GetPauseLevel() == PAUSE_ADVANCE_STEP &&
      ...
      info.GetThreadLock() != info.GetActiveCPU()->GetActiveThreadID()) {
    if (info.GetThreadLock() >= info.GetActiveCPU()->GetNumThreads()) {
      info.SetThreadLock(-1);
    } else {
      return;
    }
  }
...
void cView::DoInputs()
{
  ...
  // while( cur_char != ERR ){
    ...
    case 'Z':
      ...
      break;
    case 12: // Ideally this is CTRL-L  Hrrrm...
      Refresh();
      break;
...
void cView::ExtractCreature()
{
  ...
  cTestCPU::PrintCode(cur_gen->GetCode(), gen_name);
