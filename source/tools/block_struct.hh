// This file creates a data structure which allows indexing into an array
...
// This structure works by first creating an array where each block contains
...
// To add data to the object, call AddData(void *, int) where the first input
// is a pointer to the data, and the second is its size.  Once all the data
// has been added, a call the Process() will build up the object so it can
// be used (remember, the fixed_size must be set before Process() can run).
...
// When AddData(void *, int) is called, it will return a int value which is
...
// Remove(int) giving it back this int tag.  Remove will simply change the
...
// call to the method Compress() will do this in O(N) time.  Note that once
...
//     fragmentation.
//   BLOCK_FLAG_AUTOCALC_SIZE : This option causes Process() to automatically
...
// The following structure handles co-ordinates in terms of block number
// and offset.
...
public:
  ...
  // a Process().  The new number of cScaledBlock's used is returned.
  int Compress();
