...
void cConfig::ProcessConfiguration(int argc, char * argv[], cGenesis & genesis)
{
  ...
  if (version_id != AVIDA_VERSION) {
    fprintf(stderr, "Warning: Using incorrect genesis file.\n");
  ...
  while (arg_num < argc) {
	...
	load_pop_filename = args[arg_num];
      ...
      printf(" designed by Charles Ofria, Chris Adami, Travis Collier, and C. Titus Brown\n");
      printf(" portions of avida also coded by Travis Collier, Dennis Adler and Grace Hsu\n");
      printf(" For more information, see: http://www.krl.caltech.edu/avida/\n");
      exit(0);
