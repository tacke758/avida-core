...
double cTaskLib::TestGroup(const cTaskGroup & group, cIOBuf & inputs, 
  ...
  for( int i=0; i<group.GetNumReturnValues() && done_tasks[i] != -1; ++i ){
	...
	switch ( ctask->GetBonusType(task_count[ctask_id]) ) {
	  case '*': {
	    bonus *= ctask->GetBonus(task_count[ctask_id]);
	    break;
	  }
	  case '+': {
	    bonus += ctask->GetBonus(task_count[ctask_id]);
	  ...
	  case '^': {
	    bonus = pow(bonus, ctask->GetBonus(task_count[ctask_id]));
	    break;
	  }
	  default: {
...
void cTaskLib::AddTask(cTaskEntry & task, const cString & group_name){
  ...
  // Logic
    ...
    else if( name == "1a"  ){ index = 2; }
    else if( name == "1b"  ){ index = 3; }
    else if( name == "2a"  ){ index = 4; }
    else if( name == "2b"  ){ index = 5; }
    else if( name == "2c"  ){ index = 6; }
    else if( name == "2d"  ){ index = 7; }
    else if( name == "2e"  ){ index = 8; }
    else if( name == "2f"  ){ index = 9; }
    else if( name == "2g"  ){ index = 10; }
    else if( name == "2h"  ){ index = 11; }
