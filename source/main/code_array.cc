...
void cCodeArray::Resize(int new_size)
{
  ...
  if (new_size > max_size || new_size * 2 < max_size) {
...
void cCodeArray::Insert(int line_num, const cInstruction & new_code)
{
  ...
  if (size == max_size) {
    ...
    char * new_flags = g_memory.Get(max_size);
...
void cCodeArray::ResetSize(int new_size, int new_max)
{
  ...
  if (new_max != 0 && new_max != max_size) {
    ...
    char * new_flags = g_memory.Get(new_max);
...
void cCodeArray::Reset(int new_size, int new_max)
{
...
void cCodeArray::Reset(const cCodeArray & in_code, int offset, int new_max)
{
#ifdef DEBUG
...
int cCodeArray::FindLevenschteinDistance(const cCodeArray & in_code) const
{
