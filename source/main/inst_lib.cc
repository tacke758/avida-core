...
void cInstEntry::Init(const cString & in_name, tCPUMethod in_function,
		      int in_num_args, int in_redundancy)
{
  name = in_name;
...
void cInstLib::Add(const cGenesis & file, char * in_name,
		   tCPUMethod in_function, int in_num_args)
{
  int abundance = file.ReadInt(in_name);
...
void cInstLib::AddNop(const cGenesis & file, char * in_name, int reg)
{
  int abundance = file.ReadInt(in_name);
  if (abundance == 0) return;
...
void cInstLib::AddLabel(const cGenesis & file, char * in_name)
{
...
#else
  UCHAR id = GetInstID(in_name.GetWord(0));
...
void cInstLib::InitBinaryInst(const cGenesis & file, cInstLib & lib)
{
...
void cInstLib::InitArgsInst(const cGenesis & file, cInstLib & lib)
{
...
void cInstLib::InitRedcodeInst(const cGenesis & file, cInstLib & lib)
{
...
void cInstLib::InitBaseInst(const cGenesis & file, cInstLib & lib)
{
