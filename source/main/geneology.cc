/* geneology.cc ***************************************************************
 ...
 Time-stamp: <98/01/07 22:45:24 travc>
...
void cGeneology::LogDeath(HashIterator & it){
  ...
  if( out.good() ){
	...
	<<it.CurrentData()->birth_update<<" "           // 4 update born
	<<it.CurrentData()->death_update<<" "           // 5 update extinct
	<<it.CurrentData()->GetFitness()<<" "           // 6 fitness
	<<it.CurrentData()->depth<<" "                  // 7 depth
	<<it.CurrentData()->sub_tree_depth<<" "         // 8 sub-tree depth
	<<it.CurrentData()->num_dead_daughter_tree<<" " // 9 # sub-trees
	<<it.CurrentData()->breed_true<<" "            // 10 # breed true
	<<pit.CurrentData()->GetFitness()<<" "         // 11 parent fitness
	<<it.CurrentData()->breed_daughters<<" "       // 12 breeding daughters
	<<it.CurrentData()->worse_fit_daughters<<" "   // 13 d_fit < p_fit
	<<it.CurrentData()->same_fit_daughters<<" "    // 14 d_fit = p_fit 
	<<it.CurrentData()->better_fit_daughters<<" "  // 15 d_fit > p_fit 
	<<it.CurrentData()->d_fit_sum<<" "             // 16 daughter fit sum
	<<it.CurrentData()->d_fit_sum_square<<" ";     // 17 d fit sum sq
    for( int i=0; i<GT_NUM_DECEND; ++i ){
      out<<it.CurrentData()->decend[i]<<" ";           // 18+ decendant counts
    }
    out<<endl;
  }
}
