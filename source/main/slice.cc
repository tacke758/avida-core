...
void cSchedule::Process(int total_slices)
{
  ...
  for (total_exec = 0;
    ...
    for (int i = 0; i < num_cells; i++) {
      if (cpu_array[i].GetActiveGenotype()) cpu_array[i].SingleProcess();
    }
  }

  population->GetLocalStats()->AddExecuted(total_exec);
}
...
void cProbSchedule::Process(int total_slices)
{
  ...
  for (int i = 0; i < total_slices; i++) {
    ...
    if (next_cpu >= 0) population->SingleProcess(next_cpu);
  }

  population->GetLocalStats()->AddExecuted(total_slices);
}
...
void cProbSchedule::Adjust(cBaseCPU * in_cpu)
{
  ...
  int cpu_merit = in_cpu->GetMerit().GetInt();
...
int cLogSchedule::OK()
{
  ...
  for (i = 0; i < population->GetLocalStats()->GetNumCells(); i++) {
...
{
  int cpu_id = in_cpu->GetEnvironment()->GetID();
  int new_node = in_cpu->GetMerit().GetIntLog();
...
void cLogSchedule::Process(int total_slices)
{
  ...
  if (array_size) {
    ...
    population->GetLocalStats()->AddExecuted(i);
  }
}
...
cIntegratedScheduleNode::cIntegratedScheduleNode(cAutoAdaptivePopulation * in_population, int in_id)
{
  ...
  active_array = g_memory.GetInts(population->GetSize());
...
{
  g_memory.Free(active_array, population->GetSize());
}
...
int cIntegratedScheduleNode::OK()
{
  ...
  if (size != size_check) {
    g_debug.Error("UD %d: size = %d, but %d active nodes were found!)",
		  stats.GetUpdate(), size, size_check);
    result = FALSE;
  }
...
cIntegratedSchedule::cIntegratedSchedule(cAutoAdaptivePopulation * in_population)
{
  population = in_population;
  node_array = NULL;
  array_size = 0;
...
int cIntegratedSchedule::OK()
{
  ...
  if (!result) g_debug.Comment("cIntegratedSchedule::OK() failed at UD %d",
			       stats.GetUpdate());
...
void cIntegratedSchedule::Adjust(cBaseCPU * in_cpu)
{
  ...
  cMerit old_merit = merit_chart[in_cpu->GetEnvironment()->GetID()];
  ...
  merit_chart[in_cpu->GetEnvironment()->GetID()] = new_merit;

  // Get all of the information into a state with variables which we can
  // Mangle at will...

  UCHAR old_merit_base =  old_merit.GetBase();
  UCHAR old_merit_shift = old_merit.GetShift();
  UCHAR new_merit_base =  new_merit.GetBase();
  UCHAR new_merit_shift = new_merit.GetShift();

  // Align the old and new shifts...

  // Handle new merit being larger...
  while (old_merit_shift < new_merit_shift) {
    if (old_merit_base & 1) {
      node_array[old_merit_shift]->Remove(in_cpu->GetEnvironment()->GetID());
    }
    old_merit_shift++;
    old_merit_base >>= 1;
  }

  // Handle old merit being larger...
  while (old_merit_shift > new_merit_shift) {
    if (new_merit_base & 1) {
      if (!node_array[new_merit_shift])	InsertNode(new_merit_shift);
      node_array[new_merit_shift]->Insert(in_cpu->GetEnvironment()->GetID());
    }
    new_merit_shift++;
    new_merit_base >>= 1;
  }

  // Now old and new merit's shift should be the same size; handle remaining
  // differences.

  int old_bit, new_bit;
  while (old_merit_base || new_merit_base) {
    old_bit = old_merit_base & 1;
    new_bit = new_merit_base & 1;

    if (old_bit && !new_bit) {
      // Remove the CPU from this node...
      node_array[new_merit_shift]->Remove(in_cpu->GetEnvironment()->GetID());
    }

    if (!old_bit && new_bit) {
      // Add the CPU from this node...
      if (new_merit_shift >= array_size || !node_array[new_merit_shift])
	InsertNode(new_merit_shift);
      node_array[new_merit_shift]->Insert(in_cpu->GetEnvironment()->GetID());
    }

    // Adjust the merit info...
    new_merit_shift++;
    new_merit_base >>= 1;
    old_merit_base >>= 1;
  }
...
void cIntegratedSchedule::Process(int total_slices)
{
  ...
  if (!array_size) return;
    ...
    // And run them until for at least total_slices num of unstructions.

  int slices_used = 0;
  while (slices_used < total_slices && population->GetNumCreatures()) {
    slices_used += node_array[array_size - 1]->Process();
  }

  population->GetLocalStats()->AddExecuted(slices_used);
...
{
#ifdef DEBUG
  if (node_id < array_size && node_array[node_id])
  ...
  if (node_id >= array_size) ResizeNodes(node_id);

  node_array[node_id] = new_node;
  
  // Find the node to mark as the 'prev'.
  for (int prev_id = node_id + 1; prev_id < array_size; prev_id++) {
...
void cIntegratedSchedule::ResizeNodes(int new_max)
{
  ...
  // Construct the new array of pointers to the nodes.
  cIntegratedScheduleNode ** new_array =
    new cIntegratedScheduleNode * [new_size];

  // Copy over the old array.
  for (i = 0; i < new_size && i < array_size; i++) {
    new_array[i] = node_array[i];
  }
...
/* Randomly choose one at a time *not* using merit!
void cProbAgeSchedule::Process(int total_slices)
{
  cBaseCPU * cpu_array = population->GetCPUArray();
  int num_cells = population->GetSize();

  // Split things up between portions of the run when the soup is not
  // full, to portions of the run when it is.  This way, we have a bit of
  // a speedup when the soup fills up.

  if (num_cells == population->GetNumCreatures()) {
    for (int i = 0; i < total_slices; i++) {
      cpu_array[g_random.GetUInt(num_cells)].SingleProcess();
    }
  } else {
    cBaseCPU * cur_cpu;
    for (int i = 0; i < total_slices; i++) {
      cur_cpu = NULL;
      while (!cur_cpu || !cur_cpu->GetActiveGenotype())
	cur_cpu = &(cpu_array[g_random.GetUInt(num_cells)]);
      cur_cpu->SingleProcess();
    }
  }
}
