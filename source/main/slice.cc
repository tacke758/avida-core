...
void cProbSchedule::Process(int total_slices)
{
  ...
  for (int i = 0; i < total_slices; i++) {
    ...
    if (next_cpu >= 0) population->GetCPU(next_cpu).SingleProcess();
  }
...
void cIntegratedSchedule::Adjust(cBaseCPU * in_cpu)
{
  ...
  UINT old_merit_base  = old_merit.GetBase();
  UINT old_merit_shift = old_merit.GetShift();
  UINT new_merit_base  = new_merit.GetBase();
  UINT new_merit_shift = new_merit.GetShift();
  ...
  while (old_merit_shift < new_merit_shift) {
    if (old_merit_base & 1) {
      node_array[old_merit_shift]->Remove(cpu_id);
    }
    old_merit_shift++;
    old_merit_base >>= 1;
  }
  ...
  while (old_merit_shift > new_merit_shift) {
    if (new_merit_base & 1) {
      ...
      node_array[new_merit_shift]->Insert(cpu_id);
    }
    new_merit_shift++;
    new_merit_base >>= 1;
  }
  ...
  while (old_merit_base || new_merit_base) {
    ...
    if (old_bit && !new_bit) {
      ...
      node_array[new_merit_shift]->Remove(cpu_id);
    }

    if (!old_bit && new_bit) {
      // Add the CPU from this node...
      if (new_merit_shift >= (UINT) array_size || !node_array[new_merit_shift])
