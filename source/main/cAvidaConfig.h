...
#include "config_overrides.h"
  ...
  CONFIG_ADD_VAR(STATE_GRID_REQUIRED, bool, 0, "Organism must be in a 0 state cell in state grid to reproduce"); //JW
  CONFIG_ADD_VAR(SPIT_STATE_GRID, bool, 0, "Spit out position on state grid"); //JW
  CONFIG_ADD_VAR(GRID_NOBOUNCE, bool, 0, "1 if organism gets stuck on state grid borders"); //JW
