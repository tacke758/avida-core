...
void cGenotypeQueue::Remove(cGenotype * in_genotype)
{
  ...
  for (cur_element = root.GetNext();
    ...
    if (cur_element->GetGenotype() == in_genotype) {
      break;
    }
  }
...
cGenotype * cGenotypeQueue::Find(cCodeArray * in_code)
{
...
cGenotypeControl::cGenotypeControl(cGenebank * in_genebank)
{
  int i;
  ...
  for (i = 0; i < GENOTYPE_THREADS; i++) {
    threads[i] = NULL;
  }

  genebank = in_genebank;
}
...
cGenotype * cGenotypeControl::Find(cCodeArray * in_code)
{
  int i;
...
cSpeciesControl::cSpeciesControl(cGenebank * in_genebank)
{
  genebank = in_genebank;
  threshold =genebank->GetPopulation()->Genesis().ReadInt("SPECIES_THRESHOLD");
}
...
cSpecies * cSpeciesControl::Find(cGenotype * in_genotype, int record_level)
{
  ...
  if (record_level == SPECIES_RECORD_FULL) {
    ...
    for (int i = 0; i < size; i++) {
      cur_count = cur_species->Compare(in_genotype->GetCode(), threshold);
      if (cur_count <= threshold && cur_count < best_count) {
  ...
  if (record_level == SPECIES_RECORD_LIMITED) {
    ...
    if (cur_species &&
	cur_species->Compare(in_genotype->GetCode(), threshold) <= threshold) {
      found_species = cur_species;
    }
  }
...
void cSpeciesControl::Purge()
{
  ...
  for (int i = 0; i < garbage_queue.GetSize(); i++) {
    genebank->GetPopulation()->GetLocalStats()->RemoveSpecies(cur_species->GetID(), cur_species->GetParentID(), cur_species->GetTotalGenotypes(), cur_species->GetTotalCreatures(), cur_species->GetAge());
    cur_species = cur_species->GetNext();
  }
  garbage_queue.Purge();
}
...
cGenebank::cGenebank(cPopulation * in_population) : test_cpu(TRUE)
{ 
  int i;
  ...
  threshold          = population->Genesis().ReadInt("THRESHOLD");
  genotype_print     = population->Genesis().ReadInt("GENOTYPE_PRINT");
  species_print      = population->Genesis().ReadInt("SPECIES_PRINT");
  species_recording  = population->Genesis().ReadInt("SPECIES_RECORDING");
  genotype_print_dom = population->Genesis().ReadInt("GENOTYPE_PRINT_DOM");
...
{ 
  ...
  if (best_genotype->GetID() != prev_dom) {
    ...
    genotype_dom_time++;
    if (genotype_dom_time == genotype_print_dom) {
      char filename[40];
      sprintf(filename, "genebank/%s", best_genotype->GetName()());
 
      test_cpu.ChangeGenotype(best_genotype);
      test_cpu.TestProcess();
      test_cpu.Print(filename);
    }
  }
}

void cGenebank::GetLabel(int in_size, int in_num)
{
  char alpha[6];
  char full_name[12];
  int i;
...
cGenotype * cGenebank::AddGenotype(cCodeArray * in_code,
				   cSpecies * parent_species)
{
  ...
  if (!found_genotype) {
    ...
    population->GetLocalStats()->AddGenotype(found_genotype->GetID());
...
void cGenebank::RemoveGenotype(cGenotype * in_genotype)
{
  ...
  population->GetLocalStats()->RemoveGenotype(in_genotype->GetID(),
  ...
  if (in_genotype->GetThreshold()) {
    population->GetLocalStats()->RemoveThreshold(in_genotype->GetID());
  }
...
void cGenebank::ThresholdGenotype(cGenotype * in_genotype)
{
  ...
  if (species_recording) {
    ...
    found_species = species_control->Find(in_genotype, species_recording);
	...
	found_species->SetParentID(in_genotype->GetSpecies()->GetID());
      ...
      population->GetLocalStats()->AddSpecies(found_species->GetID());
	...
	test_cpu.ChangeGenotype(in_genotype);
	test_cpu.TestProcess();
	test_cpu.Print(filename);
      }
    } 
  ...
  if (species_recording) {
    population->GetLocalStats()->AddThreshold(in_genotype->GetID(),
					      in_genotype->GetName()(),
					      found_species->GetID());
  }
  else {
...
int cGenebank::AdjustGenotype(cGenotype * in_genotype)
{
  ...
  if ((in_genotype->GetNumCPUs() >= threshold ||
  ...
  while (cur_species != first_active) {
    ...
    if (cur_species->debug_num_genotypes != cur_species->GetNumGenotypes()) {
      g_debug.Error("UD %d: Species [%d] has %d genotypes but records %d",
		    stats.GetUpdate(), cur_species->GetID(),
...
unsigned int cGenebank::FindCRC(cCodeArray * in_code)
{
  unsigned int total = 13;
  int i;

  for (i = 0; i < in_code->GetSize(); i++) {
    total *= (*in_code)[i] + 10 + i << 6;
    total += 3;
  }
