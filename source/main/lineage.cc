...
cLineage::cLineage(double start_fitness, int parent_id, int id, double lineage_stat1, double lineage_stat2)
  : m_parent_id(parent_id), m_num_CPUs(0),
  m_total_CPUs(0), m_total_genotypes(0)
{
  if ( id < 0 )
    m_id = m_max_id++;
  else{
  ...
  m_update_born = cStats::GetUpdate();
  m_generation_born = cStats::GetAveGeneration();
...
bool cLineage::RemoveCreature(cGenotype * genotype)
{
  ...
  // did we reach zero?
  if ( m_num_CPUs == 0 ){
    // if the following assert breaks, something is wrong
    assert ( result == true );
    cStats::RemoveLineage( GetID(), GetParentID(), GetUpdateBorn(),
			   GetGenerationBorn(),
			   GetTotalCreatures(),
			   GetTotalGenotypes(),
			   GetStartFitness(), GetLineageStat1(), GetLineageStat2() );
  }
...
void cLineage::CalcCurrentFitness() const
{
  ...
  m_ave_fitness = 0;
  for ( ; it!=m_genotype_map.end(); it++ ){
    double fitness = (*it).first->GetTestFitness();
