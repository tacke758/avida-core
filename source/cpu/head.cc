...
cCPUHead cCPUHead::FindLabel(const cCodeLabel & label, int direction)
{
  ...
  if( direction < 0 ) {
    found_pos =
      FindLabel_Backwards(label, GetMemory(), GetPosition() - label.GetSize());
  }

  // Jump forwards.
  else {
    found_pos = FindLabel_Forewards(label, GetMemory(), GetPosition());
  }
...
int cCPUHead::FindLabel_Forewards(const cCodeLabel & search_label,
...
int cCPUHead::FindLabel_Backwards(const cCodeLabel & search_label,
				   const cCodeArray & search_mem, int pos)
{ 
#ifdef DEBUG
