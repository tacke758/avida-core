...
private:
  ...
  // Divide functions
    ...
    int  Divide_CheckViable(const int child_size, const int parent_size,
...
public:
  ...
  inline int GetCopyTrue() const     { return repro_data.copy_true; }
  inline int GetParentTrue() const   { return flags[CPU_FLAG_PARENT_TRUE]; }
  ...
  inline int GetStack(int depth=0) const { return hardware.GetStack(depth); }
  ...
  inline int GetFlag(int flag_num) const { return flags.GetFlag(flag_num); }
  inline cPhenotype * GetPhenotype() { return phenotype; }

#ifdef TEST_SEARCH_SIZE
  inline int GetSearchApproxSize(){ return flags[CPU_FLAG_SEARCH_APPROX_SIZE];}
#endif
