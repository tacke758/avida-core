...
class cInstSuperlibCPU : public cInstSuperlibBase {
...
public:
  ...
  cInstruction GetInst(const cString & in_name){
    ...
    return cInstSuperlibCPU::GetInstError();
  }
...
const cInstruction cInstSuperlibCPU::inst_error(255);
const cInstruction cInstSuperlibCPU::inst_default(0);
cInstSuperlibBase *cHardwareCPU::GetInstSuperlib(){ return s_inst_slib; }

cInstSuperlibCPU *cHardwareCPU::s_inst_slib = cHardwareCPU::initInstSuperlib();
cInstSuperlibCPU *cHardwareCPU::initInstSuperlib(void){
  struct cNOPEntryCPU {
  ...
  cInstSuperlibCPU *inst_superlib = new cInstSuperlibCPU(
    n_size,
    f_size,
    n_names,
    f_names,
  ...
  cout <<
  "<cHardwareCPU::initInstSuperlib> test of ick" <<endl<<
  " --- GetSize(): " << inst_superlib->GetSize() <<endl<<
  " --- GetNumNops(): " << inst_superlib->GetNumNops() <<endl<<
  " --- GetName(last): " <<
  inst_superlib->GetName(inst_superlib->GetSize() - 1) <<endl<<
  endl;

  return inst_superlib;
}
...
bool cHardwareCPU::SingleProcess_PayCosts(const cInstruction & cur_inst)
{
  ...
  if ( GetInstLib().GetCost(cur_inst) > 0 ) {
  ...
  // Prob of exec
  if ( GetInstLib().GetProbFail(cur_inst) > 0.0 ){
    return ~( g_random.P(GetInstLib().GetProbFail(cur_inst)) );
  }
#endif
  return true;
}
...
bool cHardwareCPU::SingleProcess_ExecuteInst(const cInstruction & cur_inst) 
{
  ...
  int inst_idx = GetInstLib().GetSuperlibFunctionIndex(cur_inst);
  /* XXX end */
  ...
  if (organism->TestExeErr()) inst_ptr = GetInstLib().GetRandFunction();
#endif
...
void cHardwareCPU::ReadLabel(int max_size)
{
  ...
  while (inst_lib->IsNop(inst_ptr->GetNextInst()) &&
    ...
    count++;
    inst_ptr->Advance();
    GetLabel().AddNop(inst_lib->GetNopMod(inst_ptr->GetInst()));
