...
void cHardware::InjectCode(cCodeArray * inject_code, int line_num)
{
  ...
  if (!threads[CUR_THREAD].heads[HEAD_IP].TestParasite() &&
      threads[CUR_THREAD].heads[HEAD_IP].GetPosition() > line_num)
    threads[CUR_THREAD].heads[HEAD_IP].Jump(inject_code->GetSize());
}
