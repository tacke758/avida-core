					 ...
					 int time_penalty)
{
  ...
  int time_allocated = test_info.GetAllocMod() * cpu.hardware.GetMemorySize()
    - time_penalty;

#ifdef THREAD_STATS
  tMatrix<int> thread_exe;
  if (test_info.TestThreads() == TRUE) {
    thread_exe.ResizeClear(2, cpu.GetMemorySize() * 2);
  ...
  //  static UCHAR div_id   = cpu.hardware.GetInstLib().GetInstID("c-divide");
  //  static UCHAR alloc_id = cpu.hardware.GetInstLib().GetInstID("c-alloc");
  ...
  while (time_used < time_allocated && cpu.hardware.GetMemorySize() &&
	...
	cpu.hardware.GetInstLib().GetActiveFunction(cur_inst);
      ...
      if (test_info.TestThreads() == TRUE &&
	  cpu.hardware.GetNumThreads() == 2) {
	int ip_pos = cpu.hardware.GetIPPosition();
	if (ip_pos >= thread_exe.GetNumCols()) {
	  thread_exe.Resize(2, cpu.hardware.GetMemorySize());
	}
	if (cpu.hardware.GetCurThreadID() < 2) {
  ...
  cpu.info.fitness =
    cpu.phenotype->GetCurMerit().CalcFitness(cpu.info.gestation_time);

  // Save the record of this divide.
  return cDivideRecord(divide_count + 1,
     cpu.phenotype->GetCurMerit().GetDouble(), cpu.info.gestation_time,
			    ...
			    int cur_depth)
{
  ...
  while (num_copies < 2) {
    ...
    // Case 1:
    if (test_cpu[cur_depth].cpu.info.gestation_time == 0) return FALSE;
...
void cTestCPU::PrintCode(const cCodeArray & code, cString filename)
{
  ...
  for (int j = 0; j < num_levels; j++) {
    ...
    for (i = 0; i < num_records[j]; i++)
      fp << "     Divide-" << test_info.GetDivideRecordByID(i,j)->GetDivideNum();
    if (j == 0) fp << "      Average";
    fp << endl;
    ...
    if (j == 0) fp << " " << setw(12) << test_info.CalcMerit();
    fp << endl;
    ...
    if (j == 0) fp << " " << setw(12) << test_info.CalcGestation();
    fp << endl;
    
    fp << "# Fitness.........:";
    for (i = 0; i < num_records[j]; i++)
      fp << " " << setw(12) << test_info.GetDivideRecordByID(i,j)->GetFitness();
    if (j == 0) fp << " " << setw(12) << test_info.CalcFitness();
    fp << endl;
    ...
    if (j == 0) fp << " " << setw(12) << test_info.CalcCopiedSize();
    fp << endl;

    fp << "# Executed Size...:";
    for (i = 0; i < num_records[j]; i++)
      fp << " " << setw(12) << test_info.GetDivideRecordByID(i,j)->GetExeSize();
    if (j == 0) fp << " " << setw(12) << test_info.CalcExeSize();
    fp << endl;
