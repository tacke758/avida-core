...
private:
  ...
  int cur_id;
public:
  cCPUTestInfo(int _d_tests=TEST_CPU_DIVIDES,
	       int _g_tests=TEST_CPU_GENERATIONS,
	       int _alloc_mod=TEST_ALLOC_MOD,
	       int _test_threads=FALSE) :
    divide_tests(_d_tests),
    generation_tests(_g_tests),
    alloc_mod(_alloc_mod),
  ...
  int GetDivideTests() { return divide_tests; }
  int GetGenerationTests() { return generation_tests; }
  int GetAllocMod() { return alloc_mod; }
